...
returns status "200 OK", this endpoint essentially serves as a gatekeeper for all requests
needing authentication.

All services in this project uses JWT authentication, CRSF authentication is diabled by the removal of CRSS authentication
class 'CsrfViewMiddleware' from settings file attribute MIDDLEWARE.  JWT authentication is enabled by setting the following
in 'REST_FRAMEWORK' attribute in settings file:

    'DEFAULT_AUTHENTICATION_CLASSES': (
        'rest_framework_jwt.authentication.JSONWebTokenAuthentication',
    ),

and turning on authentication by setting also the 'REST_FRAMEWORK' attribute in settings file:
...
These two settings tell Django to enable authentication for all rest endpoints and use the JWT authentication method
(as specified by 'JSONWebTokenAuthentication' in attribute 'DEFAULT_AUTHENTICATION_CLASSES')
